...
export function StoryItem({ story }: { story: Story }) {
  const avatarUrl = story.profiles?.avatar_url || story.media_url || undefined;
  const username = story.profiles?.username || "Story";
  ...
  return (
    <div className="w-16 shrink-0 text-center">
      ...
      <div className="mt-1 truncate text-[10px]" title={username}>
        {username}
      </div>
    </div>
  );
